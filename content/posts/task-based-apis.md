...
summary: This Post is concerned with a possble future direction of Application Programming
  ...
  been delivered. It proposes that there is much that we can learn from a small number
...
The thrust and direction of the Post is that of APIs should be empathetic to the business-led goals that they support and should preserve the true intent that they have been created to serve.
...
I characterise the pain points experienced and uncover some of their root causes. I offer a design philosophy expressed as a set of core values that could be applied to the design of APIs, which should cause less pain and suffering.

The set of core values when applied to APIs results in a style or design of API that I give the moniker of _Task-based APIs._ I assert that a Task-based API should _exhibit the quality of Behavioural Completeness in support of their assigned Bounded Context_.
...
* APIs that are brittle (_they break easily_)
...
For example, Windows ActiveX or COM-based Component Libraries (_spatial and platform coupling_)
...
(_known as the principle of composition_); i.e. everything became more difficult. This
...
Further complicated by most of the easily accessed examples of API creation (_and especially
examples involving Web Services_) being created by Microsoft aligned publishers who go for the _easy_
...
* A Response containing a non-specific error message like "_Can't process badly formed Request_" and if you're lucky an application status code
...
These are often associated with question like "_how do I achieve `<state business objective here>`,
when all I can do is update the Customer?_" As the Message Payload is merely a projection of the
...
goal to _update a Customer_? No. Instead, there is in mind a sound business objective, _in other words
the true intent_, which has the effect that one or several facets of a Customer's state need to
change. Unfortunately, the original intent is lost because it has been translated into merely
_update_. This also means that verifying the "_true intent_" by the Service Implementation is
...
things like XSDs do most of the checking for us, as well as supplying usable and useful
documentation about the message payload. Usually, we are not that competent
...
Sometimes successfully, and sometimes not, but it is always hard. Such code become dark
...
was often out of the reach of many Teams and so "_competent developers_" were used instead;
resulting in sub-optimal outcomes.

The principle of composition adopted by the WS Standards was that Web Service Designs
could incorporate only those WS Standards that they needed to achieve the goals. Resulting in
further damage to Platform Independence. Projects that involved Web Services would have
...
making it inevitable that these negotiations were long, hard and full of contradictions,
...
XML, had already solved the problems they were experiencing and that they just needed to use the
...
write a RESTful Service withiout resorting to the auto-scaffolding and code generation
...
Now look at the impact of adding intent back into the Sales Order Service API. The only change we will make, for illustration purposes, is to add a new Service Method called `CancelOrder`. The `CancelOrder` Method requires a Sales Document Number argument to know which Sales Order to cancel. Given this change, how do we now feel about the following questions?
...
sterile documents we have previously produced (_for those few occasions
where we did create documentation!_).